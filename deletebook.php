...
if(isset($_GET['bookid'])) {
    ...
    $con = new mysqli('localhost', 'root', '', 'student');
	$queryi="select * from books_members where bookid='$id'";
...
while($row=$result->fetch_assoc()){
 ...
 echo "<tr><td>".$row["bookid"]."</td><td>".$row["booktitle"]."</td><td>".$row["bookauthor"]."</td><td>".$row["bookisbn"]."</td><td align='center'><a class='btn btn-danger' href ='deletebook.php?bookid=".$row["bookid"]." '>
...
function update(str){
	...
	if (str.length == 0) { 
        ...
        xmlhttp.open("GET", "deletebook.php?bookingid=" + str+"&isbnbook="+isbns+"&titlebook="+titles+"&authorbook="+authors, true);
        xmlhttp.send();
    }
